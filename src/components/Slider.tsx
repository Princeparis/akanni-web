...
const Slider = (): React.JSX.Element => {
  ...
  const createSlide = (slideIndex: number): HTMLDivElement => {
  ...
  const splitText = (slide: HTMLDivElement): void => {
  ...
  const animateSlide = (direction: 'up' | 'down'): void => {
    ...
    if (direction === 'down') {
      currentSlideRef.current =
        currentSlideRef.current === totalSlides ? 1 : currentSlideRef.current + 1
    } else {
      currentSlideRef.current =
        currentSlideRef.current === 1 ? totalSlides : currentSlideRef.current - 1
    }
    ...
    setTimeout(() => {
      ...
      if (slider) slider.appendChild(newSlide)
  ...
  useEffect(() => {
    ...
    if (slider) slider.appendChild(initialSlide)
